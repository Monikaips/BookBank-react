...
function RetrieveBooks() {
    ...
    const booksRef = useRef(null); // Ref for scrolling to books section
    ...
    const fetchBooks = async (e) => {
        ...
        // Check if no books were found and alert the user
        if (fetchedBooks.length === 0) {
            alert('No books found.');
        }
    };

    // Automatically scroll to the books section if books are available
    useEffect(() => {
        if (books.length > 0) {
    ...
    return (
        <div id="container">
            ...
            <div id="books-container" ref={booksRef}>
                {loading ? (
                    <p style={{
